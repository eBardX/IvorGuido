...
extension GMNPitchTests {
    ...
    func accidental_allCases() {
        let cases: [GMNPitch.Accidental] = [.doubleFlat, .flat, .omitted, .sharp, .doubleSharp]
    ...
    func init_differentOctaves() {
        let pitch1 = GMNPitch(name: .c, accidental: .omitted, octave: 0)
        let pitch2 = GMNPitch(name: .c, accidental: .omitted, octave: 5)

        #expect(pitch1.octave == 0)
        #expect(pitch2.octave == 5)
        #expect(pitch1 != pitch2)
    }

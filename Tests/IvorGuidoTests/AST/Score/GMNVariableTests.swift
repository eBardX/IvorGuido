...
extension GMNVariableTests {
    ...
    // A parsed string variable carries a declaration-time pre-parse in
    // `symbols`; one built via the public initializer does not. Equality is
    // defined over `name` and `value` only, so the two must still match.
    @Test
    func equatable_ignoresSymbols() throws {
        let input = "$seq = \"a/4 b\"; [ c ]"
        let (score, _) = try GMNParser().parse(Data(input.utf8))
        let parsed = score.variables[0]
        let constructed = GMNVariable(name: parsed.name,
                                      value: parsed.value)

        #expect(parsed.symbols != nil)
        #expect(constructed.symbols == nil)
